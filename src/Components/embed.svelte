<script>
  ...
  const shortenString = (string) =>
    string.length > "I Know What You Did Last".length
  ...
  {:else}
    <Stack orientation="horizontal" class="playholder">
      ...
      <Stack padding_left="large">
        <Heading class="song-title" align="left" as="h4" is="strong"
          >{shortenString(info.name)}</Heading
        >
